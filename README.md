...
FPGAI is an open-source compiler framework that converts ONNX neural network models into deployable FPGA/SoC accelerator projects, with automatic HLS generation, runtime packaging, and correctness benchmarking.

The project is designed for:

* researchers building custom FPGA AI pipelines,
* engineers deploying models on Xilinx SoCs,
* contributors adding new operators, dataflows, and benchmarking features.
...
If you do not yet have a `requirements.txt`, install at least:

```bash
pip install numpy onnx onnxruntime pyyaml
```
...
FPGAI can be used in two main ways.
...
Add the HLS layer implementation under:
