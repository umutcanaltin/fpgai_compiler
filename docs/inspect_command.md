...
The bug fixed in this update was CLI disupdate order: the legacy top-level `--config`
check ran before explicit subcommand disupdate. Therefore `fpgai inspect --config ...`
...
The fix disupdates explicit subcommands first and uses legacy top-level `--config`
