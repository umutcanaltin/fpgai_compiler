...
def _graph() -> Graph:
    graph = Graph("mixed_hls_vhdl_hls")
    graph.inputs = ["input"]
    graph.outputs = ["output"]
    for name in ("input", "scaled", "vhdl_out", "output"):
        graph.add_tensor(name, (1,), "int16")
    graph.add_op("Scale2", ["input"], ["scaled"], name="hls_pre")
    graph.add_op("IdentityVHDL", ["scaled"], ["vhdl_out"], name="vhdl_mid")
    graph.add_op("Add1", ["vhdl_out"], ["output"], name="hls_post")
    return graph


def main() -> int:
    ap = argparse.ArgumentParser(description="Validate graph-driven HLS -> VHDL -> HLS physical composition")
    ap.add_argument("--out", default="build/graph_mixed_backend_validation")
    ...
    post_rtl = _run_hls_stage(
        root,
        name="add1_hls",
        source=Path("examples/mixed_backend/add1_hls.cpp").resolve(),
        part=args.part,
        period_ns=args.clock_period_ns,
        vitis_hls=args.vitis_hls,
    )
    ...
    project = emit_graph_mixed_backend_physical_project(
        GraphMixedBackendPhysicalRequest(
            ...
            graph=graph,
            bindings={
                "hls_pre": HLSPhysicalBinding("hls_pre", pre_rtl, "scale2_hls"),
                "vhdl_mid": VHDLPhysicalBinding("vhdl_mid", vhdl),
                "hls_post": HLSPhysicalBinding("hls_post", post_rtl, "add1_hls"),
            },
            part=args.part,
