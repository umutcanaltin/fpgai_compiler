...
from fpgai.benchmark.plots import generate_benchmark_plot_artifacts
...
def test_benchmark_plot_generator_creates_inference_and_training_sections(tmp_path: Path) -> None:
    ...
    manifest = generate_benchmark_plot_artifacts([inference, training], output_dir=tmp_path / "benchmark_plots")
    ...
    assert (tmp_path / "benchmark_plots/figures/figure_01_inference_hls_latency.svg").exists()
    assert (tmp_path / "benchmark_plots/figures/figure_08_training_curve_loss.svg").exists()

    with (tmp_path / "benchmark_plots/data/hardware_knob_settings.csv").open(newline="", encoding="utf-8") as f:
        rows = list(csv.DictReader(f))
    assert len(rows) == 4
    assert rows[0]["yaml_path"] == "optimization.parallel.pe"


def test_benchmark_plot_generator_marks_missing_real_runtime_as_pending(tmp_path: Path) -> None:
    inference = _base_out(tmp_path, "inference_model", "inference")
    training = _base_out(tmp_path, "training_model", "training_on_device")

    manifest = generate_benchmark_plot_artifacts([tmp_path], output_dir=tmp_path / "benchmark_plots")
    ...
    assert (tmp_path / "benchmark_plots/benchmark_plot_manifest.md").exists()


def test_benchmark_plot_generator_extracts_metrics_from_real_report_files(tmp_path: Path) -> None:
...
""".strip(),
    ...
    manifest = generate_benchmark_plot_artifacts([out], output_dir=tmp_path / "benchmark_plots")
    ...
    with (tmp_path / "benchmark_plots/data/benchmark_plot_rows.csv").open(newline="", encoding="utf-8") as f:
        rows = list(csv.DictReader(f))
...
def test_benchmark_plot_generator_writes_benchmark_tables_and_extra_resource_figures(tmp_path: Path) -> None:
    training = _base_out(tmp_path, "training_model", "training_on_device")
    ...
    manifest = generate_benchmark_plot_artifacts([training], output_dir=tmp_path / "benchmark_plots")
    ...
    assert (tmp_path / "benchmark_plots/tables/table_01_experiment_overview.md").exists()
    assert (tmp_path / "benchmark_plots/tables/table_04_vivado_results.csv").exists()
    ...
    table_md = (tmp_path / "benchmark_plots/benchmark_plot_manifest.md").read_text(encoding="utf-8")
    assert "## Benchmark tables" in table_md
    assert "table_06_pending_measurements" in table_md


def test_benchmark_plot_generator_prefers_frozen_benchmark_runs_and_skips_runtime_packages(tmp_path: Path) -> None:
    old = _base_out(tmp_path / "build" / "examples", "old_example", "inference")
    benchmark = _base_out(tmp_path / "build" / "benchmark_runs" / "inference", "I0_baseline_fx16_embedded", "inference")
    runtime_pkg = benchmark / "runtime_package"
    ...
    manifest = generate_benchmark_plot_artifacts([tmp_path / "build"], output_dir=tmp_path / "benchmark_plots")

    assert manifest["compile_output_count"] == 1
    with (tmp_path / "benchmark_plots/tables/table_01_experiment_overview.csv").open(newline="", encoding="utf-8") as f:
        rows = list(csv.DictReader(f))
    assert [row["design_id"] for row in rows] == ["I0_baseline_fx16_embedded"]


def test_benchmark_table_uses_compact_precision_label(tmp_path: Path) -> None:
    out = _base_out(tmp_path / "benchmark_runs" / "inference", "I1_precision_fx8_embedded", "inference")
    _write_json(out / "reports/hardware_knob_contract.json", {"knobs": [
        {
    ...
    generate_benchmark_plot_artifacts([tmp_path / "benchmark_runs"], output_dir=tmp_path / "benchmark_plots")
    text = (tmp_path / "benchmark_plots/tables/table_01_experiment_overview.md").read_text(encoding="utf-8")
    assert "fx8_3" in text
    assert "total_bits" not in text


def test_benchmark_plot_generator_writes_captions_summary_and_gallery(tmp_path: Path) -> None:
    inference = _base_out(tmp_path / "build" / "benchmark_runs" / "inference", "I0_baseline_fx16_embedded", "inference")
    variant = _base_out(tmp_path / "build" / "benchmark_runs" / "inference", "I1_precision_fx8_embedded", "inference")
    ...
    for rel in [
        ...
        "figure_captions.md",
        "table_captions.md",
        "benchmark_observations_from_artifacts.md",
        "plot_gallery.md",
        "plot_gallery.html",
        "tables/table_08_result_comparisons.md",
        "tables/table_08_result_comparisons.csv",
    ]:
        assert (tmp_path / "benchmark_plots" / rel).exists()

    summary = (tmp_path / "benchmark_plots/benchmark_results_summary.md").read_text(encoding="utf-8")
    assert "Key computed comparisons" in summary
    assert "I1_precision_fx8_embedded" in summary
    manifest_md = (tmp_path / "benchmark_plots/benchmark_plot_manifest.md").read_text(encoding="utf-8")
    assert "## Narrative files" in manifest_md
    assert "table_08_result_comparisons" in manifest_md


def test_benchmark_plot_generator_writes_expanded_comparisons_classifications_and_subsection(tmp_path: Path) -> None:
    root = tmp_path / "build" / "benchmark_runs"
    ...
    assert "benchmark_results_subsection_md" in manifest["narrative_files"]
    comparison_path = tmp_path / "benchmark_plots" / "tables" / "table_08_result_comparisons.csv"
    ...
    summary_text = (tmp_path / "benchmark_plots" / "tables" / "table_09_result_classification_summary.md").read_text(encoding="utf-8")
    assert "Result classification summary" in summary_text
    assert "expected_tradeoff" in summary_text
    subsection_text = (tmp_path / "benchmark_plots" / "benchmark_results_subsection.md").read_text(encoding="utf-8")
    assert "Benchmark-ready results subsection draft" in subsection_text
    assert "Claim boundary" in subsection_text


def test_benchmark_plot_generator_writes_numeric_validation_tables_and_figures(tmp_path: Path) -> None:
    root = tmp_path / "build" / "benchmark_runs"
    ...
    _write_json(
        fx16 / "reports" / "numeric_validation.json",
        {
            ...
            "validation_claim_allowed": {"numeric_correctness": True},
        },
    )
    _write_json(
    ...
    with (tmp_path / "benchmark_plots" / "data" / "numeric_validation_rows.csv").open(newline="", encoding="utf-8") as f:
        rows = list(csv.DictReader(f))
    ...
    subsection = (tmp_path / "benchmark_plots" / "benchmark_results_subsection.md").read_text(encoding="utf-8")
