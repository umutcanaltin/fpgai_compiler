...
from fpgai.reporting.validation_trace import build_validation_trace, write_validation_trace
...
def test_validation_trace_trace_keeps_generated_vivado_project_separate_from_vivado_implementation_passed(tmp_path: Path) -> None:
    out = _minimal_example(tmp_path, "vivado_project", vivado_project=True)
    chain = build_validation_trace([out])
    ...
    vivado_claim = next(c for c in example["records"] if c["record_id"] == "vivado_project_handoff_generated")
...
def test_validation_trace_trace_training_claim_is_static_not_runtime_convergence(tmp_path: Path) -> None:
    out = _minimal_example(tmp_path, "training", pipeline_mode="training_on_device")
    chain = build_validation_trace([out])
    example = chain["examples"][0]
    training_claim = next(c for c in example["records"] if c["record_id"] == "training_pipeline_materialized")
    assert training_claim["validation_ready"] is True
...
def test_validation_trace_trace_marks_failed_movement_as_not_validation_ready(tmp_path: Path) -> None:
    ...
    claim = next(c for c in chain["examples"][0]["records"] if c["record_id"] == "data_movement_contract_materialized")
    assert claim["validation_ready"] is False
    assert "reports/movement_contract_validation.json:passed" in claim["required_validation"]



def test_validation_trace_trace_does_not_fail_when_vivado_not_requested_placeholder_exists(tmp_path: Path) -> None:
    out = _minimal_example(tmp_path, "cpp_only", vivado_project=False)
    chain = build_validation_trace([out])
    example = chain["examples"][0]
    assert chain["passed"] is True
    assert all(c["record_id"] != "vivado_project_handoff_generated" for c in example["records"])
    assert example["artifact_status"]["vivado_bd_validation"] == "not_requested"

def test_validation_trace_trace_writes_json_and_markdown(tmp_path: Path) -> None:
    out = _minimal_example(tmp_path, "example")
    json_path = tmp_path / "validation_trace_trace.json"
    md_path = tmp_path / "validation_trace_trace.md"
    chain = write_validation_trace([out], json_path, md_path)
    assert chain["passed"] is True
    loaded = json.loads(json_path.read_text(encoding="utf-8"))
    assert loaded["artifact_kind"] == "validation_trace_trace"
    md = md_path.read_text(encoding="utf-8")
    assert "FPGAI Benchmark Validation Trace" in md
