...
def _read_rows(path: Path) -> list[dict[str, str]]:
    assert path.exists(), f"missing expected benchmark result artifact: {path}"
    with path.open(newline="") as f:
        return list(csv.DictReader(f))


def test_benchmark_artifact_index_coverage_if_generated() -> None:
    p = Path("benchmark_results/index/benchmark_artifact_index.csv")
    if not p.exists():
        return
...
def test_benchmark_numeric_joined_coverage_if_generated() -> None:
    p = Path("benchmark_results/parsed/benchmark_numeric_joined.csv")
    if not p.exists():
        return
...
def test_benchmark_tables_coverage_if_generated() -> None:
    base = Path("benchmark_results/tables")
    if not base.exists():
        return
...
def test_benchmark_figures_if_generated() -> None:
    base = Path("benchmark_results/figures")
    if not base.exists():
        return
...
def test_publication_tables_if_generated() -> None:
    base = Path("benchmark_results/tables")
    if not base.exists():
        return

    expected = [
        "table_publication_artifact_coverage.tex",
        "table_publication_prediction_vs_hls.tex",
        "table_publication_vivado_subset.tex",
        "table_publication_knob_summary.tex",
        "table_publication_training_capacity.tex",
    ]

    for name in expected:
        path = base / name
        assert path.exists(), f"missing publication table: {path}"
        text = path.read_text()
        assert "\\begin{table}" in text
        assert "\\end{table}" in text
        assert "textbackslash" not in text

    training = (base / "table_publication_training_capacity.tex").read_text()
