...
from fpgai.benchmark.experiment_setup import generate_experiment_setup_artifacts


MATRIX = Path("benchmark_runs/benchmark_matrix.yml")


def _load_matrix() -> dict:
    if not MATRIX.exists():
        pytest.skip("optional benchmark pack is not present in this repository archive")
    return yaml.safe_load(MATRIX.read_text(encoding="utf-8"))


def test_benchmark_matrix_has_controlled_inference_and_training_rows() -> None:
    matrix = _load_matrix()
...
def test_benchmark_configs_are_valid_and_match_sections() -> None:
    matrix = _load_matrix()
    for row in matrix["experiments"]:
        ...
        metadata = cfg.raw.get("metadata", {}).get("benchmark", {})
...
def test_generate_benchmark_setup_artifacts(tmp_path: Path) -> None:
    if not MATRIX.exists():
        pytest.skip("optional benchmark pack is not present in this repository archive")
    ...
    with (tmp_path / "setup/benchmark_setup_rows.csv").open(newline="", encoding="utf-8") as f:
...
def test_compile_command_plan_preserves_compile_config_token_boundary(tmp_path: Path) -> None:
    ...
    assert "continuing to remaining benchmark rows" in selected_sh

    assert "T8_real_fpga_training_curve_candidate" in plan_md
    assert "python -m fpgai.cli compile --config benchmark_runs/training/T8_real_fpga_training_curve_candidate.yml" in plan_md
...
def test_validate_compile_artifacts_rejects_failed_hls_for_vivado_stage(tmp_path: Path) -> None:
    from fpgai.benchmark.experiment_setup import validate_compile_artifacts
    ...
    (out / "reports/benchmark_result_row.json").write_text(
        json.dumps({
