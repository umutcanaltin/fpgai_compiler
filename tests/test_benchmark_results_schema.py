...
from fpgai.reporting.benchmark_results import MASTER_RESULT_FIELDS, build_master_result_row, write_master_results, write_schema_json
...
def test_write_master_results_outputs_json_csv_md_and_schema(tmp_path: Path) -> None:
    ...
    results = write_master_results(
        [out],
        output_json=tmp_path / "benchmark_results" / "master_results.json",
        output_csv=tmp_path / "benchmark_results" / "master_results.csv",
        output_md=tmp_path / "benchmark_results" / "master_results.md",
        schema_json=tmp_path / "benchmark_results" / "schema" / "master_result_schema.json",
        schema_md=tmp_path / "benchmark_results" / "schema" / "master_result_schema.md",
    )
    ...
    data = json.loads((tmp_path / "benchmark_results" / "master_results.json").read_text(encoding="utf-8"))
    assert data["artifact_kind"] == "benchmark_master_results"
    with (tmp_path / "benchmark_results" / "master_results.csv").open(newline="", encoding="utf-8") as f:
        rows = list(csv.DictReader(f))
    assert rows[0]["design_id"] == "demo"
    schema = json.loads((tmp_path / "benchmark_results" / "schema" / "master_result_schema.json").read_text(encoding="utf-8"))
    assert schema["artifact_kind"] == "benchmark_master_result_schema"
