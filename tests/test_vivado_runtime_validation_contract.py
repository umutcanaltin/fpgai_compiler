...
def test_vivado_bd_contract_runtime_api_and_claim_audit_for_cpp_only_inference(tmp_path: Path) -> None:
    ...
    raw.setdefault("project", {})["out_dir"] = str(tmp_path / "runtime_validation_inference")
    ...
    assert "validation_boundary" in bd
    ...
    assert "board-specific" in manifest["runtime_api"]["validation_boundary"]
    ...
    assert feature["validation_ready"] is False
...
def test_vivado_bd_contract_for_m_axi_training_ports_and_runtime_api(tmp_path: Path) -> None:
    ...
    raw.setdefault("project", {})["out_dir"] = str(tmp_path / "runtime_validation_training")
...
def test_vivado_implementation_stage_reports_missing_tool_without_fake_success(tmp_path: Path) -> None:
    pytest.importorskip("onnx")
    raw = copy.deepcopy(_load_inference_config())
    raw.setdefault("project", {})["out_dir"] = str(tmp_path / "vivado_impl_validation")
    raw.setdefault("weights", {})["mode"] = "embedded"
    raw.setdefault("runtime", {})["sequence"] = ["run_inference"]
    ...
    manifest = json.loads((out_dir / "manifest.json").read_text(encoding="utf-8"))
    assert manifest["vivado_validation_artifacts"]["vivado_implementation_report_json"].endswith("vivado_implementation_report.json")


def test_bitstream_stage_reports_missing_artifact_without_fake_success(tmp_path: Path) -> None:
    pytest.importorskip("onnx")
    raw = copy.deepcopy(_load_inference_config())
    raw.setdefault("project", {})["out_dir"] = str(tmp_path / "vivado_bitstream_validation")
    raw.setdefault("weights", {})["mode"] = "embedded"
    raw.setdefault("runtime", {})["sequence"] = ["run_inference"]
