"""Benchmark plot data/figure generation from generated FPGAI reports.
...
from fpgai.reporting.benchmark_results import build_master_result_row

SCHEMA_VERSION = 1
ARTIFACT_KIND = "fpgai_benchmark_plot_artifacts"
...
def _precision_family(value: Any) -> str:
    """Return a compact benchmark-table precision label.

    The raw reports often store the full precision mapping as a dictionary.
    That is useful for JSON/CSV traceability, but it is unreadable in benchmark
...
def _benchmark_clean_row(row: Mapping[str, Any]) -> dict[str, Any]:
    cleaned = dict(row)
...
def _benchmark_sort_key(row: Mapping[str, Any]) -> tuple[int, int, str]:
...
def _benchmark_table_rows(plot_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for raw in plot_rows:
        row = _benchmark_clean_row(raw)
        rows.append({
    ...
    return sorted(rows, key=_benchmark_sort_key)
...
def _write_benchmark_tables(table_dir: Path, *, plot_rows: Sequence[Mapping[str, Any]], knob_rows: Sequence[Mapping[str, Any]], curve_rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    tables["table_01_experiment_overview"] = _write_table_pair(
        table_dir, stem="table_01_experiment_overview", title="Experiment overview", rows=_benchmark_table_rows(plot_rows),
...
def _numeric_validation_row(out_dir: Path, row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    benchmark_claim = _nested_mapping(payload, "validation_claim_allowed").get("numeric_correctness")
    ...
    return {
        ...
        "numeric_comparison_eligible": benchmark_claim if benchmark_claim not in (None, "") else False,
...
def _numeric_summary_rows(numeric_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    return [dict(row) for row in sorted(numeric_rows, key=_benchmark_sort_key)]
...
def _training_numeric_rows(numeric_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    return sorted([dict(row) for row in numeric_rows if row.get("section") == "training"], key=_benchmark_sort_key)
...
def _inference_task_quality_rows(numeric_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    return sorted([dict(row) for row in numeric_rows if row.get("section") == "inference"], key=_benchmark_sort_key)
...
def _training_task_quality_rows(numeric_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in _training_numeric_rows(numeric_rows):
        ...
        if row.get("numeric_quality") != "passed":
            ...
            reason = "training update differs from reference but preserves usable gradient/weight artifacts"
...
def _training_curve_rows(out_dir: Path, design_id: str) -> list[dict[str, Any]]:
    candidates = [
        out_dir / "reports" / "training_curve.csv",
...
def _status_svg(statuses: Mapping[str, str], out_path: Path) -> Path:
    ...
    lines = _svg_axes(width, height, "Benchmark plot manifest", "Created plots and pending real-measurement inputs")
    y = 88
...
def _is_benchmark_dir(path: Path) -> bool:
    return "benchmark_runs" in path.parts
...
def _is_probably_compile_output(path: Path) -> bool:
    ...
    if path.name in {"reports", "plots", "experiment_setup", "benchmark_results"}:
        return False
    return True
...
def _discover_out_dirs(paths: Iterable[str | Path]) -> list[Path]:
    ...
    # the frozen benchmark matrix, default to that matrix instead of
    # mixing in stale scratch/example outputs.  This keeps benchmark tables focused
    # while preserving backward compatibility for directories without
    # benchmark_runs outputs.
    benchmark_rows = [p for p in out if _is_benchmark_dir(p)]
    if benchmark_rows:
        return sorted(benchmark_rows, key=lambda p: p.as_posix())
    return sorted(out, key=lambda p: p.as_posix())


def generate_benchmark_plot_artifacts(out_dirs: Iterable[str | Path], *, output_dir: str | Path = "benchmark_results/plots") -> dict[str, Any]:
    """Build benchmark plot data, SVG figures, and a manifest from compile outputs."""
    out = Path(output_dir)
    data_dir = out / "data"
    fig_dir = out / "figures"
    ...
    plot_rows = sorted((_plot_row(path) for path in compile_dirs), key=_benchmark_sort_key)
    ...
    _write_csv(data_dir / "benchmark_plot_rows.csv", plot_rows, PLOT_ROW_FIELDS)
    ...
    table_files = _write_benchmark_tables(table_dir, plot_rows=plot_rows, knob_rows=knob_rows, curve_rows=curve_rows)
    ...
    table_files["table_10_numeric_validation_summary"] = _write_table_pair(
        ...
        fields=["section", "design_id", "precision", "numeric_validation_status", "numeric_quality", "numeric_comparison_eligible", "max_abs_error", "mae", "rmse", "cosine_similarity", "gradient_cosine", "weight_after_cosine", "weight_delta_cosine", "loss_validation_status"],
        labels={"numeric_validation_status": "numeric status", "numeric_comparison_eligible": "claim allowed", "max_abs_error": "max abs error", "cosine_similarity": "cosine", "gradient_cosine": "gradient cosine", "weight_after_cosine": "weight-after cosine", "weight_delta_cosine": "weight-delta cosine"},
    )
    ...
    manifest = {
        ...
        "data_files": {
            "benchmark_plot_rows_csv": (data_dir / "benchmark_plot_rows.csv").as_posix(),
    ...
    _write_json(out / "benchmark_plot_manifest.json", manifest)
    _write_md(out / "benchmark_plot_manifest.md", manifest)
    return manifest
...
def _comparison_interpretation(definition: Mapping[str, str], classification: str, rows: Sequence[Mapping[str, Any]]) -> str:
    ...
    if classification == "no_observable_effect":
        return "No observable HLS/Vivado metric change in the current frozen artifacts; use as coverage/deployability artifacts, not as a performance-resource claim."
...
def _comparison_rows(plot_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    """Return benchmark-ready pairwise comparisons for the frozen matrix.

    The comparison set is deliberately explicit rather than inferred from file
    names.  This keeps the benchmark narrative stable and makes every claim tied to
...
def _figure_caption_rows(created: Mapping[str, str], pending: Mapping[str, str]) -> list[dict[str, Any]]:
    captions = {
        "figure_00_plot_status": "Artifact-status overview showing which benchmark figures are generated and which require real board-runtime measurements.",
        ...
        "figure_12_all_hls_lut": "HLS LUT comparison across all frozen benchmark designs with HLS reports.",
        "figure_13_all_vivado_power": "Vivado power comparison across all frozen benchmark designs with implementation reports.",
        "figure_14_all_vivado_wns": "Vivado timing slack comparison across all frozen benchmark designs with implementation reports.",
...
def _table_caption_rows(table_files: Mapping[str, Any]) -> list[dict[str, Any]]:
    captions = {
        "table_01_experiment_overview": "Overview of frozen benchmark rows, pipeline mode, compact precision label, and artifact support status.",
        "table_02_design_knobs": "Resolved YAML/hardware knob settings and where each knob was applied in the compiler/hardware flow.",
        "table_03_hls_results": "Vitis HLS synthesis metrics used for benchmark HLS latency/resource comparisons.",
        "table_04_vivado_results": "Vivado implementation metrics used for benchmark resource, timing, and power comparisons.",
        ...
        "table_08_result_comparisons": "Pairwise benchmark comparisons computed from the frozen experiment subset, including automatic result classification and interpretation.",
        "table_09_result_classification_summary": "Summary count of comparison classifications used to separate strong results, expected tradeoffs, deployability rows, no-observable-effect rows, and pending runtime records.",
...
def _benchmark_results_subsection_text(
    ...
    lines = [
        "# Benchmark-ready results subsection draft",
        "",
        "## Results",
        "",
        (
            f"The frozen FPGAI benchmark matrix contains {inference_count} inference design rows and "
    ...
    if numeric_rows:
        ...
        lines.append(
            f"- Inference numeric validation: {inf_passed}/{len(inference_numeric)} row(s) passed or contain reference-output comparison artifacts."
        )
        lines.append(
            f"- Training numeric validation: {train_passed}/{len(training_numeric)} row(s) passed or contain gradient/weight-update comparison artifacts."
        )
    ...
    lines.append(
        "The generated comparisons support HLS/Vivado artifact records, deployability/package records, "
        "and explicit no-observable-effect classifications. They do not yet support measured FPGA "
        "latency, measured energy, or measured training convergence records. Those records require "
...
def _write_captions_and_summary(
    ...
    *,
    ...
    records_md = out_dir / "benchmark_observations_from_artifacts.md"
    summary_md = out_dir / "benchmark_results_summary.md"
    gallery_md = out_dir / "plot_gallery.md"
    gallery_html = out_dir / "plot_gallery.html"
    subsection_md = out_dir / "benchmark_results_subsection.md"
    ...
    claim_lines = [
        "# Validation records from artifacts",
        "",
        "These records are generated only from existing FPGAI reports and runtime artifacts.",
        "",
    ...
    numeric_rows = list(numeric_rows or [])
    numeric_passed = sum(1 for row in numeric_rows if row.get("numeric_quality") == "passed")
    claim_lines.append(f"- HLS synthesis metrics are available for `{hls_count}` frozen benchmark row(s).")
    claim_lines.append(f"- Vivado implementation metrics are available for `{vivado_count}` frozen benchmark row(s).")
    claim_lines.append(f"- Numeric-validation rows are available for `{len(numeric_rows)}` frozen benchmark row(s), with `{numeric_passed}` row(s) marked passed.")
    ...
    lines = [
        "# FPGAI benchmark results summary",
        "",
        ...
        "- Use `tables/table_10_numeric_validation_summary.md`, `table_11_inference_precision_numeric_tradeoff.md`, and `table_12_training_numeric_validation.md` for numeric behavior records.",
    ...
    lines.extend([
        ...
        "- Use `benchmark_observations_from_artifacts.md` to avoid over-claiming runtime behavior.",
        "- Open `plot_gallery.html` locally to inspect all generated SVG plots in one page.",
    ])
    summary_md.write_text("\n".join(lines) + "\n", encoding="utf-8")

    subsection_md.write_text(_benchmark_results_subsection_text(plot_rows, representative_rows, classification_rows, numeric_rows), encoding="utf-8")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate FPGAI benchmark plot data and SVG figures from compile outputs.")
    ...
    manifest = generate_benchmark_plot_artifacts(args.out_dirs, output_dir=args.output_dir)
