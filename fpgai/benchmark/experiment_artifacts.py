...
These reports normalize per-compile artifact status into benchmark-facing rows.
...
def _validation_summary_payload(out_dir: Path) -> dict[str, Any]:
    return _read_json(out_dir / "reports" / "validation_summary.json")
...
def _claim_level(row: Mapping[str, Any]) -> str:
    """Derive the strongest defensible validation claim from artifact status.

    Higher-level hardware records must be gated by successful HLS synthesis.  A
    stale or externally copied Vivado/bitstream artifact must not upgrade a row
    if the current compile's HLS result failed.  This keeps benchmark tables from
...
def _artifact_row(out_dir: Path, manifest: Mapping[str, Any]) -> dict[str, Any]:
    benchmark = _validation_summary_payload(out_dir)
    validation_flags = benchmark.get("validation_flags", {}) if isinstance(benchmark.get("validation_flags"), Mapping) else {}
    ...
    vivado_implemented = bool(validation_flags.get("vivado_implemented") or _get(benchmark, "validation_flags.vivado_implemented"))
    ...
    bitstream_generated = bool(validation_flags.get("bitstream_generated") or vivado_bridge.get("bitstream_exists"))
...
def _training_curve_rows(out_dir: Path, manifest: Mapping[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    Compile-time Python/HLS reference data is correctness artifacts, but it is
    not a real FPGA training curve. Benchmark training curves must come from a
...
def _update_package_manifest(out_dir: Path, artifacts: Mapping[str, Any]) -> None:
    ...
    copy_map = {
        ...
        "benchmark_result_row_json": (out_dir / "reports" / "benchmark_result_row.json", package_dir / "reports" / "benchmark_result_row.json"),
        "benchmark_result_row_csv": (out_dir / "reports" / "benchmark_result_row.csv", package_dir / "reports" / "benchmark_result_row.csv"),
        ...
        "training_curve_csv": (out_dir / "reports" / "training_curve.csv", package_dir / "reports" / "training_curve.csv"),
        "training_curve_summary_json": (out_dir / "reports" / "training_curve_summary.json", package_dir / "reports" / "training_curve_summary.json"),
    }
...
def emit_experiment_artifact_reports(out_dir: str | Path) -> dict[str, Any]:
    ...
    matrix_payload = {
        ...
        "row_count": 1,
        "rows": [row],
        "claim_boundary": "The claim_level is derived from generated artifacts and reports. Real board-execution records require board_runtime_report.json.",
    }
    ...
    _write_json(reports / "benchmark_result_row.json", row)
    _write_csv(reports / "benchmark_result_row.csv", [row], _EXPERIMENT_FIELDS)
    ...
    if curve_summary.get("available"):
        ...
        report_payload = {
            ...
            "training_curve_csv": "reports/training_curve.csv",
        ...
        _write_csv(reports / "training_curve.csv", curve_rows, _TRAINING_CURVE_FIELDS)
        _write_json(reports / "training_curve_summary.json", curve_summary)
        training_paths = {
            ...
            "training_curve_csv": "reports/training_curve.csv",
            "training_curve_summary_json": "reports/training_curve_summary.json",
        }
    else:
        contract_payload = {
            "schema_version": 1,
    ...
    summary = {
        ...
        "paths": {
            ...
            "benchmark_result_row_json": "reports/benchmark_result_row.json",
            "benchmark_result_row_csv": "reports/benchmark_result_row.csv",
            **training_paths,
        },
    }
