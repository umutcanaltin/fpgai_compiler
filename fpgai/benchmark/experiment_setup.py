...
def _current_artifact_row(out_dir: str | Path) -> dict[str, Any]:
    out = Path(out_dir)
    direct = _read_json(out / "reports" / "benchmark_result_row.json")
    if direct:
        return direct
...
def validate_compile_artifacts(
    ...
    *,
    ...
    still emit reports after a tool failure for inspection, but benchmark
...
def _write_command_script(path: Path, *, title: str, rows: list[dict[str, Any]], extra_lines: list[str] | None = None) -> None:
    """Write a resilient shell plan for benchmark compilation.

    Benchmark matrices should not stop at the first failed design: a single Vivado
    ...
    lines.extend([
        "set -u",
        "set -o pipefail",
        "FPGAI_EXPERIMENT_SETUP_DIR=${FPGAI_EXPERIMENT_SETUP_DIR:-benchmark_results/experiment_setup}",
        ...
        '    python -m fpgai.benchmark.experiment_setup --validate-artifact "$project_out_dir" --configured-stage "$configured_stage" --expected-claim-level "$expected_claim_level" >> "$log" 2>&1',
        '    artifact_status=$?',
        '  fi',
        ...
        '  if [ "$status" -ne 0 ]; then',
        '    echo "[FPGAI][WARN] ${experiment_id} failed with status ${status}; continuing to remaining benchmark rows."',
    ...
    lines.extend([
        ...
        "  echo \"[FPGAI][SUMMARY] $FPGAI_COMPILE_FAILURES benchmark compile row(s) failed. Inspect $FPGAI_COMPILE_LOG_DIR.\"",
        "  exit 1",
        "fi",
        "echo \"[FPGAI][SUMMARY] All selected benchmark compile rows passed.\"",
    ])
    path.write_text("\n".join(lines).rstrip() + "\n", encoding="utf-8")
...
def _row_from_experiment(
    *,
    ...
    metadata_id = _get_path(cfg, "metadata.benchmark.id", None) if cfg else None
    if config_loadable and metadata_id != row.get("id"):
        issues.append({
            "id": row.get("id"),
            "severity": "warning",
            "message": "metadata.benchmark.id does not match matrix id",
        })
...
def generate_experiment_setup_artifacts(
    ...
    *,
    ...
    if not isinstance(experiments, list):
        raise RuntimeError("benchmark matrix must contain experiments: []")
    ...
    _write_csv(output / "benchmark_setup_rows.csv", rows, table_fields)
    _write_markdown_table(output / "benchmark_setup_rows.md", rows, table_fields)
    ...
    setup_command = f"python -m fpgai.benchmark.experiment_setup {matrix_path.relative_to(repo_root_path) if matrix_path.is_relative_to(repo_root_path) else matrix_path} --output-dir {output}"
    plot_command = "python -m fpgai.benchmark.plots build --output-dir benchmark_results/plots"
    common_header = [
        setup_command + " || exit 1",
        "",
        "# Compile commands are generated from benchmark_runs/benchmark_matrix.yml.",
        "# Board-runtime rows compile deployable packages only; real board execution is a separate measured step.",
    ]
    _write_command_script(output / "compile_command_plan.sh.txt", title="FPGAI full benchmark command plan", rows=command_rows, extra_lines=common_header + ["# Full matrix."])
    _write_command_script(output / "compile_selected_smoke.sh.txt", title="FPGAI selected benchmark smoke compile plan", rows=selected_smoke_rows, extra_lines=common_header + ["# Recommended first subset before the full matrix."])
    _write_command_script(output / "compile_inference_matrix.sh.txt", title="FPGAI inference benchmark compile plan", rows=inference_rows, extra_lines=common_header + ["# Inference section rows."])
    _write_command_script(output / "compile_training_matrix.sh.txt", title="FPGAI training benchmark compile plan", rows=training_rows, extra_lines=common_header + ["# Training section rows."])
    ...
    (output / "regenerate_plots.sh.txt").write_text("\n".join(["# Regenerate benchmark tables and figures", "set -e", plot_command]) + "\n", encoding="utf-8")
    ...
    manifest: dict[str, Any] = {
        ...
        "issues": issues,
        "tables": {
            "setup_rows_csv": str(output / "benchmark_setup_rows.csv"),
            "setup_rows_md": str(output / "benchmark_setup_rows.md"),
...
def _manifest_markdown(manifest: dict[str, Any]) -> str:
    lines: list[str] = ["# FPGAI benchmark setup", ""]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate FPGAI benchmark setup reports.")
    parser.add_argument("matrix", nargs="?", help="benchmark matrix YAML")
    parser.add_argument("--output-dir", default="benchmark_results/experiment_setup", help="output directory")
