...
def _find_first(root: Path, name: str) -> Optional[Path]:
    if not root.exists():
        return None
    preferred = root / "training_hls_outputs" / name
...
def _source_markers(hls_src: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "has_preload_mode": "mode == 0" in src or "mode==0" in src or "mode 0" in src,
        "has_emit_weights_mode": "mode == 1" in src or "mode==1" in src or "mode 1" in src,
        "has_train_mode": "mode == 2" in src or "mode==2" in src or "target_buf" in src,
        "has_sgd_update": "sgd_update" in src,
        "has_backward": "backward" in src or "dW_" in src,
        "has_weight_stream": "aux" in src and ("read_f32" in src or "aux_stream" in src),
    }


def _bench_files(artifact_root: Path) -> Dict[str, bool]:
    return {
        "hls_grads_bin": _find_first(artifact_root, "grads.bin") is not None,
        "hls_weights_before_bin": _find_first(artifact_root, "weights_before.bin") is not None,
        "hls_weights_after_bin": _find_first(artifact_root, "weights_after.bin") is not None,
    }
...
def collect(root: Path) -> List[Dict[str, Any]]:
    ...
    for item in results.get("results", []):
        ...
        row.update(_bench_files(artifact_root))
...
def write_table(rows: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    fields = [
        ...
        "has_train_mode", "has_sgd_update", "has_backward", "hls_grads_bin",
        "hls_weights_before_bin", "hls_weights_after_bin", "grad_cosine", "grad_mae",
        "grad_max_abs", "weight_after_cosine", "weight_after_mae", "weight_after_max_abs",
        "weight_delta_cosine", "weight_delta_mae", "weight_delta_max_abs", "forward_layers",
        "backward_layers", "param_grad_weight_layers", "param_grad_bias_layers",
    ]
    (out_dir / "training_accelerator_artifacts.json").write_text(json.dumps(rows, indent=2), encoding="utf-8")
    with (out_dir / "training_accelerator_artifacts.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=fields, extrasaction="ignore")
        w.writeheader()
        for row in rows:
            w.writerow(row)

    lines = ["# Training accelerator artifacts", "", "| " + " | ".join(fields) + " |", "|" + "---|" * len(fields)]
    for row in rows:
        lines.append("| " + " | ".join(_fmt(row.get(k)) for k in fields) + " |")
    (out_dir / "training_accelerator_artifacts.md").write_text("\n".join(lines) + "\n", encoding="utf-8")

    all_pass = rows and all(r.get("status") == "passed" for r in rows)
    all_compare = rows and all(r.get("training_compare") for r in rows)
    ...
    claim_rows = [
        {
            "claim": "On-device training HLS path emits forward/backward/update kernel",
            "status": "supported" if all_pass and has_modes else "partial",
            "artifacts": "mode 0 preload, mode 1 weights snapshot, mode 2 training step",
        },
        {
            "claim": "HLS training artifacts include gradients and before/after weights for all evaluated designs",
            "status": "supported" if all_pass and has_all_bins else "partial",
            "artifacts": "grads.bin, weights_before.bin, weights_after.bin generated by C simulation",
        },
        {
            "claim": "Training HLS step is compared against Python reference for all evaluated designs",
            "status": "supported" if all_pass and all_compare else "partial",
            "artifacts": "training_compare/results.json with gradient and weight-update metrics",
        },
        {
            "claim": "Full multi-epoch training convergence on hardware",
            "status": "not claimed by this sprint",
            "artifacts": "this sprint validates one training step; convergence remains a future experiment",
        },
    ]
    (out_dir / "training_claim_support.json").write_text(json.dumps(claim_rows, indent=2), encoding="utf-8")
    claim_lines = ["# Training claim-support status", "", "| claim | status | artifacts |", "|---|---|---|"]
    for r in claim_rows:
        claim_lines.append(f"| {r['claim']} | {r['status']} | {r['artifacts']} |")
...
def main() -> int:
    ...
    out = root / "training_accelerator_artifacts"
    rows = collect(root)
    write_table(rows, out)
    print((out / "training_accelerator_artifacts.md").read_text(encoding="utf-8"))
