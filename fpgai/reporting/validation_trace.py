#!/usr/bin/env python3
"""Build a benchmark validation trace for compiled FPGAI examples.

The report links user/compiler knobs to generated artifacts, validation reports,
and validation boundaries.  It intentionally separates generated-artifact artifacts from
...
def _claim_build_backend(out_dir: Path, manifest: dict[str, Any], reports: dict[str, dict[str, Any]]) -> ValidationRecord:
    stages = _build_stages(manifest)
    requested = [f"build.stages.{k}={v}" for k, v in sorted(stages.items())]
    missing: list[str] = []
    artifacts = ["manifest.json:configuration.effective.build_stages"]
    if stages.get("cpp") and _exists(out_dir, "hls/src/deeplearn.cpp"):
        artifacts.append("hls/src/deeplearn.cpp")
    elif stages.get("cpp"):
    ...
    if stages.get("runtime_package") and _exists(out_dir, "runtime_package/package_manifest.json"):
        artifacts.append("runtime_package/package_manifest.json")
    elif stages.get("runtime_package"):
        missing.append("runtime_package/package_manifest.json")
    return ValidationRecord(
        record_id="build_backend_artifacts_materialized",
        ...
        validation_statement="FPGAI materialized the requested compile-stage artifacts and recorded the effective build-stage decisions in the manifest.",
...
def _claim_movement(out_dir: Path, reports: dict[str, dict[str, Any]]) -> ValidationRecord:
    ...
    return ValidationRecord(
        record_id="data_movement_contract_materialized",
        ...
        validation_level=VALIDATION_STATIC_ARTIFACT,
        validation_ready=safe,
        validation_statement="FPGAI generated and statically validated PS-PL movement paths against the resolved data-movement plan and runtime package.",
...
def _claim_knobs(out_dir: Path, reports: dict[str, dict[str, Any]]) -> ValidationRecord:
    ...
    safe = bool(knobs)
    return ValidationRecord(
        record_id="hardware_knob_decisions_traced",
        ...
        validation_level=VALIDATION_STATIC_ARTIFACT,
        validation_ready=safe,
        validation_statement="FPGAI records hardware-relevant YAML knobs, their effective compiler decisions, and the generated artifact locations where those decisions are materialized.",
...
def _claim_estimates(reports: dict[str, dict[str, Any]]) -> ValidationRecord:
    present = [name for name in ("resource_prediction", "timing_prediction", "board_fit") if reports.get(name)]
    return ValidationRecord(
        record_id="compiler_estimates_available_with_validation_boundary",
        feature="estimation",
        ...
        validation_ready=bool(present),
        validation_statement="FPGAI emits compiler-estimated resource/timing/board-fit reports with a separate validation boundary from HLS and Vivado measurements.",
...
def _claim_training(out_dir: Path, manifest: dict[str, Any], reports: dict[str, dict[str, Any]]) -> ValidationRecord | None:
    ...
    return ValidationRecord(
        record_id="training_pipeline_materialized",
        feature="training",
        ...
        validation_level=VALIDATION_STATIC_ARTIFACT,
        validation_ready=not missing,
        validation_statement="FPGAI generated and statically validated an on-device training HLS artifact with label, weight, and gradient movement paths.",
...
def _claim_vivado(out_dir: Path, manifest: dict[str, Any], reports: dict[str, dict[str, Any]], levels: dict[str, bool]) -> ValidationRecord | None:
    stages = _build_stages(manifest)
    ...
    # status=not_requested.  That placeholder is useful status artifacts, but it
    ...
    return ValidationRecord(
        record_id="vivado_project_handoff_generated",
        ...
        validation_ready=not missing,
        validation_statement="FPGAI generated a Vivado project handoff and structurally validated the Tcl/BD generation for the requested interface topology.",
...
def _claim_validation_boundary(levels: dict[str, bool]) -> ValidationRecord:
    missing = []
    ...
    return ValidationRecord(
        record_id="validation_boundary_declared",
        ...
        validation_statement="FPGAI explicitly separates compiler/static validation from HLS, Vivado, bitstream, and board runtime validation.",
        pending_validation=["compiler/static validation is enough for measured performance records", "generated Tcl proves Vivado implementation"],
        required_validation=missing,
    )
...
def build_validation_trace(out_dirs: Iterable[Path]) -> dict[str, Any]:
    examples = [build_example_validation(Path(p)) for p in out_dirs]
    failed_examples = [e.example for e in examples if any(c.validation_ready is False for c in e.records if c.record_id not in {"compiler_estimates_available_with_validation_boundary"})]
    chain = ValidationTraceReport(
        artifact_kind="validation_trace_trace",
        schema_version=1,
        ...
        summary={
            ...
            "record_count": sum(len(e.records) for e in examples),
        },
        examples=examples,
...
def render_markdown(chain: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# FPGAI Benchmark Validation Trace")
    lines.append("")
    ...
    lines.append(f"- Claims: `{summary.get('record_count')}`")
    lines.append(f"- Failed examples: `{summary.get('failed_example_names', [])}`")
    lines.append("")
    lines.append("## Example validation status")
    lines.append("")
    lines.append("| Example | Validation level | Compiler/static | HLS validation | Vivado validation | Bitstream validation | Runtime validation |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|")
    ...
    for ex in chain.get("examples", []):
        ...
        lines.append("")
        lines.append("| Record | Feature | Validation level | Benchmark-ready | Benchmark statement | Missing validation artifacts |")
        lines.append("|---|---|---|---:|---|---|")
        for claim in ex.get("records", []):
            missing = "; ".join(claim.get("required_validation", []))
            safe = str(claim.get("validation_statement", "")).replace("|", "\\|")
            lines.append(
                f"| {claim.get('record_id')} | {claim.get('feature')} | {claim.get('validation_level')} | {_md_bool(bool(claim.get('validation_ready')))} | {safe} | {missing} |"
            )
        lines.append("")
    return "\n".join(lines) + "\n"


def write_validation_trace(out_dirs: Iterable[Path], output_json: Path, output_md: Path | None = None) -> dict[str, Any]:
    chain = build_validation_trace(out_dirs)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build FPGAI benchmark validation trace")
    ...
    chain = write_validation_trace([Path(p) for p in args.out_dirs], Path(args.output_json), Path(args.output_md) if args.output_md else None)
