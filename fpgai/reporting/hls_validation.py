...
def _md_from_report(payload: Mapping[str, Any]) -> str:
    lines = ["# HLS Synthesis Report", "", f"Status: {payload.get('status')}", f"Validation-qualified: {payload.get('validation_ready')}"]
...
def _md_from_comparison(payload: Mapping[str, Any]) -> str:
    lines = ["# Estimate vs HLS", "", f"Status: {payload.get('status')}", f"Validation-qualified: {payload.get('validation_ready')}"]
    reason = payload.get("reason")
    if reason:
        lines.extend(["", f"Reason: {reason}"])
...
def emit_hls_validation_reports(
    *,
    out_dir: str | Path,
    ...
    hls_payload: dict[str, Any] = {
        ...
        "validation_ready": status == "parsed",
