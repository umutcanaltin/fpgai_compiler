...
SCHEMA_VERSION = 1
ARTIFACT_KIND = "benchmark_master_results"
...
def write_schema_json(path: str | Path) -> Path:
    schema = {
        "schema_version": SCHEMA_VERSION,
        "artifact_kind": "benchmark_master_result_schema",
        "fields": [
...
def write_schema_md(path: str | Path) -> Path:
    lines = ["# FPGAI Benchmark Master Result Schema", "", "| Field | Type |", "|---|---|"]
    for field in MASTER_RESULT_FIELDS:
...
def _write_md(path: Path, rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    lines = ["# FPGAI Benchmark Master Results", "", f"Rows: {len(rows)}", "", "| " + " | ".join(preview_fields) + " |", "|" + "|".join(["---"] * len(preview_fields)) + "|"]
    for row in rows:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build FPGAI benchmark master results from existing compile reports.")
    parser.add_argument("out_dirs", nargs="*", help="FPGAI compile output directories.")
    parser.add_argument("--output-json", default="benchmark_results/master_results.json")
    parser.add_argument("--output-csv", default="benchmark_results/master_results.csv")
    parser.add_argument("--output-md", default="benchmark_results/master_results.md")
    parser.add_argument("--schema-json", default="benchmark_results/schema/master_result_schema.json")
    parser.add_argument("--schema-md", default="benchmark_results/schema/master_result_schema.md")
