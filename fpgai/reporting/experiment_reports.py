...
def write_training_learning_ablation_reports(
    ...
    """Write canonical benchmark-facing summaries for training ablation sweeps.
    ...
    for record in results_payload.get("results", []):
        ...
        rows.append({
            ...
            "benchmark_eligible": eligible and not confounders,
    ...
    fields = [
        ...
        "comparison_passed", "claim_scope", "board_fit_status", "benchmark_eligible",
    ...
    for row in rows:
        lines.append(
            "| `{design_name}` | {comparison_role} | {batch_size} | {record_visits_executed} | {optimizer_updates} | {record_visits_per_update} | {loss_reduction} | {loss_reduction_per_record_visit} | {loss_reduction_per_optimizer_update} | {final_accuracy} | {numeric_validation_status} | {benchmark_eligible} |".format(
    ...
    lines.extend([
        ...
        "- Small-sample smoke workloads are never promoted to convergence/generalization records.",
    ])
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    ...
    eligibility_path.write_text(json.dumps({
        ...
        "comparison_basis": comparison_basis,
        "row_count": len(rows),
        "eligible_count": sum(1 for row in rows if row["benchmark_eligible"]),
...
def write_paired_training_batch_ablation_reports(
    ...
    """Combine already-generated ablation summaries without merging records."""
    ...
    fields = [
        ...
        "claim_scope", "benchmark_eligible", "confounders", "source_experiment_dir",
    ]
