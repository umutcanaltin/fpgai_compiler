...
# Sprint 11D source fix: override after original definition so runtime modes
# link both the loader helpers and fpgai_params.cpp runtime W/B storage.
def emit_csim_tcl(
    *,
    top_name: str = "deeplearn",
    part: str,
    ...
    top_cflags = f'-I${{INC_DIR}} -I${{LAYERS_INC_DIR}}{extra_cflags}'
    tb_cflags = f'-I${{INC_DIR}} -I${{LAYERS_INC_DIR}}{extra_cflags}'
    common_cflags = f'-I${{INC_DIR}} -I${{LAYERS_INC_DIR}}{extra_cflags}'

    if normalized_mode in {"stream", "streamed", "ddr", "dma_ddr"}:
        weight_src_lines = (
            f'add_files ${{SRC_DIR}}/weights_runtime.cpp -cflags "{common_cflags}"\n'
            f'add_files ${{SRC_DIR}}/fpgai_params.cpp -cflags "{common_cflags}"'
        )
    else:
        weight_src_lines = f'add_files ${{SRC_DIR}}/fpgai_params.cpp -cflags "{common_cflags}"'
