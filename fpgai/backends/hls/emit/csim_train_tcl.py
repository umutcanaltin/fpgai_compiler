...
if {{[file exists "${{SRC_DIR}}/layers/dense.cpp"]}} {{ add_files ${{SRC_DIR}}/layers/dense.cpp -cflags "{top_cflags}" }}
if {{[file exists "${{SRC_DIR}}/layers/conv.cpp"]}} {{ add_files ${{SRC_DIR}}/layers/conv.cpp -cflags "{top_cflags}" }}
if {{[file exists "${{SRC_DIR}}/layers/pool.cpp"]}} {{ add_files ${{SRC_DIR}}/layers/pool.cpp -cflags "{top_cflags}" }}
if {{[file exists "${{SRC_DIR}}/layers/activations.cpp"]}} {{ add_files ${{SRC_DIR}}/layers/activations.cpp -cflags "{top_cflags}" }}
if {{[file exists "${{SRC_DIR}}/layers/batchnorm.cpp"]}} {{ add_files ${{SRC_DIR}}/layers/batchnorm.cpp -cflags "{top_cflags}" }}
add_files -tb ${{SRC_DIR}}/tb.cpp -cflags "{tb_cflags}"
open_solution -reset sol1
set_part {part}
create_clock -period 5.0 -name default
file mkdir "{out_arg}"
csim_design -clean -argv "{input_bin_path} {target_bin_path} {out_arg}"
csynth_design
