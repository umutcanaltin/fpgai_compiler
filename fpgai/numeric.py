...
def _compare_optimizer_state_pair(ref_path: Any, got_path: Any, *, lsb: float) -> dict[str, Any]:
    """Compare canonical packed optimizer state using the fixed-point LSB contract."""
    ref = _read_f32_file(ref_path)
    got = _read_f32_file(got_path)
    payload: dict[str, Any] = {
        ...
        "ref_exists": _exists(ref_path),
        "got_exists": _exists(got_path),
        "numeric_tolerance": {"kind": "optimizer_state_lsb", "lsb": float(lsb), "allowed_lsb": 1},
    }
    if ref is None or got is None:
    ...
    within_one_lsb = sum(1 for value in abs_diffs if value <= float(lsb) + 1.0e-15)
    same_shape = len(ref) == len(got)
    passed = bool(same_shape and n == len(ref) and within_one_lsb == len(ref))
    payload.update(metrics)
    payload.update({
        ...
        "all_words_within_one_lsb": bool(same_shape and within_one_lsb == len(ref)),
    ...
    if isinstance(comparisons_cfg, dict):
        for name, cfg in sorted(comparisons_cfg.items()):
            ...
            cmp_payload = _compare_optimizer_state_pair(
                cfg.get("ref"), cfg.get("got"), lsb=_optimizer_state_lsb(raw_config)
            )
            comparisons[str(name)] = cmp_payload
            if cmp_payload.get("status") == "compared":
                any_compared = True
...
def emit_numeric_validation_report(
    ...
    *,
    ...
    optimizer_json_path.write_text(json.dumps(optimizer_state_validation, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    optimizer_lines = [
