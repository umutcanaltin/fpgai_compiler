    ...
    backend_path.write_text(
        "\n".join(
            [
                ...
                "    mapping = {'float32': np.float32, 'float': np.float32, 'float16': np.float16, 'bfloat16': getattr(np, 'bfloat16', np.uint16), 'uint8': np.uint8, 'int8': np.int8, 'uint16': np.uint16, 'int16': np.int16, 'uint32': np.uint32, 'int32': np.int32, 'uint64': np.uint64, 'int64': np.int64, 'bool': np.bool_}",
                ...
                "        if logical.startswith('state_cursor__'):",
                "            state_name = logical[len('state_cursor__'):].replace('/', '_').replace('.', '_')",
                "            aliases = [f'fpgai_state_{state_name}_cursor', logical]",
                "        elif logical.startswith('state__'):",
                "            state_name = logical[len('state__'):].replace('/', '_').replace('.', '_')",
                "            aliases = [f'fpgai_state_{state_name}', logical]",
                "        else:",
                "            aliases = {",
                ...
                "        return result",
                "",
                "    def reset_state(self, name: str | None = None) -> Any:",
                "        if hasattr(self.backend, 'reset_state'):",
                "            return self.backend.reset_state(name=name)",
                "        raise RuntimeError('Backend does not implement persistent-state reset.')",
                "",
                "    def import_state(self, payload: Any, *, name: str | None = None) -> Any:",
                "        if hasattr(self.backend, 'import_state'):",
                "            return self.backend.import_state(payload, name=name)",
                "        if hasattr(self.backend, 'write_state'):",
                "            return self.backend.write_state(payload, name=name)",
                "        raise RuntimeError('Backend does not implement persistent-state import/write.')",
                "",
                "    def export_state(self, *, name: str | None = None) -> Any:",
                "        if hasattr(self.backend, 'export_state'):",
                "            return self.backend.export_state(name=name)",
                "        if hasattr(self.backend, 'read_state'):",
                "            return self.backend.read_state(name=name)",
                "        raise RuntimeError('Backend does not implement persistent-state export/read.')",
                "",
                "    def read_state(self, *, name: str | None = None) -> Any:",
                "        if hasattr(self.backend, 'read_state'):",
                "            return self.backend.read_state(name=name)",
                "        if hasattr(self.backend, 'export_state'):",
                "            return self.backend.export_state(name=name)",
                "        raise RuntimeError('Backend does not implement persistent-state read/export.')",
                "",
                "    def write_state(self, payload: Any, *, name: str | None = None) -> Any:",
                "        if hasattr(self.backend, 'write_state'):",
                "            return self.backend.write_state(payload, name=name)",
                "        if hasattr(self.backend, 'import_state'):",
                "            return self.backend.import_state(payload, name=name)",
                "        raise RuntimeError('Backend does not implement persistent-state write/import.')",
                "",
                "    def _state_buffer_name(self, name: str) -> str:",
                "        logical = f'state__{name}'",
                "        if logical not in self.buffers:",
                "            raise RuntimeError(f'No external persistent-state buffer is bound for {name!r}.')",
                "        return logical",
                "",
                "    def _state_names(self) -> list[str]:",
                "        return [str(key)[7:] for key in self.buffers if str(key).startswith('state__')]",
                "",
                "    def reset_state(self, name: str | None = None) -> Any:",
                "        if hasattr(self.backend, 'reset_state'):",
                "            return self.backend.reset_state(name=name)",
                "        names = [str(name)] if name is not None else self._state_names()",
                "        if not names:",
                "            raise RuntimeError('Backend does not implement persistent-state reset and no external state buffers are bound.')",
                "        for state_name in names:",
                "            logical = self._state_buffer_name(state_name)",
                "            buf = self.buffers[logical]",
                "            try:",
                "                buf[...] = 0",
                "            except Exception as exc:",
                "                raise TypeError(f'Persistent-state buffer {state_name!r} cannot be zeroed.') from exc",
                "            _sync_to_device(buf)",
                "            cursor_logical = f'state_cursor__{state_name}'",
                "            if cursor_logical in self.buffers:",
                "                cursor = self.buffers[cursor_logical]",
                "                try:",
                "                    cursor[...] = 0",
                "                except Exception as exc:",
                "                    raise TypeError(f'Persistent-state cursor {state_name!r} cannot be zeroed.') from exc",
                "                _sync_to_device(cursor)",
                "        return {'reset': names}",
                "",
                "    def write_state(self, payload: Any, *, name: str | None = None) -> Any:",
                "        if hasattr(self.backend, 'write_state'):",
                ...
                "            return self.backend.import_state(payload, name=name)",
                "        if name is None:",
                "            raise ValueError('write_state requires name=... when using generated external-state buffers.')",
                "        logical = self._state_buffer_name(str(name))",
                "        buf = self.buffers[logical]",
                "        try:",
                "            import numpy as np",
                "            target = buf.reshape(-1) if hasattr(buf, 'reshape') else buf",
                "            if isinstance(payload, (bytes, bytearray, memoryview)):",
                "                values = np.frombuffer(bytes(payload), dtype=getattr(buf, 'dtype', np.uint8))",
                "            else:",
                "                values = np.asarray(payload, dtype=getattr(buf, 'dtype', None)).reshape(-1)",
                "            if len(values) != len(target):",
                "                raise ValueError(f'Persistent-state payload for {name!r} has {len(values)} elements; expected {len(target)}.')",
                "            target[:] = values",
                "        except ValueError:",
                "            raise",
                "        except Exception as exc:",
                "            raise TypeError(f'Could not write persistent-state buffer {name!r}.') from exc",
                "        _sync_to_device(buf)",
                "        return {'written': str(name)}",
                "",
                "    def import_state(self, payload: Any, *, name: str | None = None) -> Any:",
                "        return self.write_state(payload, name=name)",
                "",
                "    def read_state(self, *, name: str | None = None) -> Any:",
                "        if hasattr(self.backend, 'read_state'):",
                "            return self.backend.read_state(name=name)",
                ...
                "            return self.backend.export_state(name=name)",
                "        if name is None:",
                "            return {state_name: self.read_state(name=state_name) for state_name in self._state_names()}",
                "        logical = self._state_buffer_name(str(name))",
                "        buf = self.buffers[logical]",
                "        _sync_from_device(buf)",
                "        if hasattr(buf, 'copy'):",
                "            return buf.copy()",
                "        return bytes(buf) if isinstance(buf, (bytes, bytearray, memoryview)) else memoryview(buf).tobytes()",
                "",
                "    def export_state(self, *, name: str | None = None) -> Any:",
                "        return self.read_state(name=name)",
                "",
